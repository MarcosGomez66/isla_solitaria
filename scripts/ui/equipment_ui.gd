...
func draw_inventory():
	...
	var inv_sort = InvManager.sort_by_prio(['weapon', 'tool', 'throwable', 'equipment'])
	...
	for i in inv_sort:
		...
		card.top_pressed.connect(EqManager.equip)
...
func draw_spaces(container, data: Stack):
	...
	card.top_pressed.connect(EqManager.unequip)
	
func draw_main_space():
	draw_spaces(main_space_con, EqManager.slots['main'])
	
func draw_armor_space():
	draw_spaces(armor_space_con, EqManager.slots['armor'])
	
func draw_backpack_space():
	draw_spaces(backpack_space_con, EqManager.slots['backpack'])
