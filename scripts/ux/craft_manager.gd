...
func _ready() -> void:
	load_recipes()
	InvManager.inventory_changed.connect(check_ingredients)
...
func load_recipes():
	var dir = DirAccess.open("res://resources/recipes/hands")
	if dir == null:
		push_error('recipe folder not found')
		
	for file in dir.get_files():
		if file.ends_with('.tres'):
			var recipe = load("res://resources/recipes/hands/" + file)
...
func check_ingredients():
	...
	for recipe in hand_recipes:
		if compare_ing(InvManager.get_ingredients(), recipe.ingredients):
			var score = 0
...
func get_craft_status():
	...
	if InvManager.get_ingredients().is_empty():
...
func consume_ingredients():
	...
	for ing in _current_recipe.ingredients:
		for ing2 in InvManager.get_ingredients():
